...
class DeliveryEnvironment(object):
    ...
    def _generate_q_values(self, box_size=0.2):
        ...
        # Generate actual Q Values corresponding to time elapsed between 2 pts
    ...
    def render(self, return_img=False):
        ...
        # Show START
        if len(self.stops) > 0:
            xy: tuple[float, float] = self._get_xy(True)  # type: ignore
        ...
        if len(self.stops) > 1:
            ax.plot(
                self.x[self.stops], self.y[self.stops], c="blue", linewidth=1,
                linestyle="--"
                )
        ...
        if return_img:
            ...
            fig.canvas.draw()
            assert isinstance(fig.canvas, FigureCanvasTkAgg)
    ...
    def format_schedule(self) -> str:
        ...
        cost = 0
        prev = 0
        for next in range(1, len(self.stops)):
            prev_city = int(self.stops[prev])
            next_city = int(self.stops[next])
            assert isinstance(prev_city, int)
            assert isinstance(next_city, int)

            cost += (curr_cost := self.q_stops[prev_city, next_city])
    ...
    def _get_reward(self, state, new_state):
        ...
        if self.method == "distance":
            ...
            intersections = self._calculate_box_intersection(
                xs,
                xe,
                ys,
                ye,
...
class DeliveryQAgent(QAgent):
    ...
    def act(self, s):
        ...
        if np.random.rand() > self.epsilon:
            ...
            a = np.random.choice(
                [
                    x for x in range(self.actions_size)
                    if x not in self.states_memory
                ])
